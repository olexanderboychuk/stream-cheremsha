  ...
  <head>
    ...
    <style>
      ...
      .coinIcon.coinPulse {{
        ...
        animation: coinPulse 1.2s cubic-bezier(0.42, 0, 0.58, 1) infinite;
      }}
      @keyframes coinPulse {{
    ...
    <script>
      (function() {{
        ...
        function coinSvg(px) {{
          ...
          const r = Math.max(2, Math.round(s * 0.36));
          const cx = s / 2;
          const cy = s / 2;
          return (
            '<svg class="coinIcon" viewBox="0 0 24 24" width="' +
            s +
            '" height="' +
            s +
            ...
            Math.max(8, Math.round(r * 1.05)) +
            '" font-weight="900" fill="#854d0e" font-family="system-ui,sans-serif">◆</text>' +
            '</svg>'
          );
        }}
        ...
        function leadersSortedForDisplay(raw, modeRaw) {{
          ...
          arr.sort((a, b) => {{
            const la = clampInt((a && a.coins), 0, 2000000000, 0);
            const lb = clampInt((b && b.coins), 0, 2000000000, 0);
        ...
        function patchLeaderDom(list) {{
          ...
          const showMedal = !!cfg.show_top3_medal;
          const showCrown = !!cfg.show_top1_crown;
          const showCoin = !!cfg.show_heart;
          const coinAnimated = cfg.heart_animated !== undefined ? !!cfg.heart_animated : true;
          const coinPx = clampInt(cfg.heart_size_px, 8, 48, 14);
          ...
          for (let i = 0; i < list.length; i++) {{
            ...
            const coins = clampInt(L.coins, 0, 2000000000, 0);
            ...
            const lr = textCol.querySelector('.coinsRow');
            if (lr) {{
              lr.style.fontSize = Math.max(12, Math.round(fs * 0.78)) + 'px';
              lr.style.color = ptsCol;
              lr.style.textShadow = lshOn ? ('0 1px 0 ' + lshCol) : 'none';
              const num = lr.querySelector('.tg-coins-num');
              if (num) num.textContent = fmtInt(coins);
              if (showCoin) {{
                let h = lr.querySelector('.coinIcon');
                if (!h) {{
                  lr.insertAdjacentHTML('afterbegin', coinSvg(coinPx));
                  h = lr.querySelector('.coinIcon');
                  if (coinAnimated && h) h.classList.add('coinPulse');
                }} else {{
                  h.setAttribute('width', String(coinPx));
                  h.setAttribute('height', String(coinPx));
                  h.classList.toggle('coinPulse', !!coinAnimated);
                }}
              }} else {{
                const hx = lr.querySelector('.coinIcon');
                if (hx) hx.remove();
              }}
            }}
          }}
        ...
        function render(opts) {{
          ...
          for (let i = 0; i < list.length; i++) {{
            ...
            if (showLikes) {{
              ...
              lr.style.textShadow = lshOn ? ('0 1px 0 ' + lshCol) : 'none';
              if (showCoin) {{
                lr.insertAdjacentHTML('beforeend', coinSvg(coinPx));
                if (coinAnimated) {{
                  const h = lr.querySelector('.coinIcon');
                  if (h) h.classList.add('coinPulse');
                }}
              }}
              const num = document.createElement('span');
              num.className = 'tg-coins-num';
              num.textContent = fmtInt(coins);
              lr.appendChild(num);
